...
function audioBufferToBlob (audioBuffer: AudioBuffer, audioType: string) {
...
ipcRenderer.on('question-data', async (_, q) => {
    ...
    const normalizeAsync = async (audioBuffer : AudioBuffer) => {
    ...
    // arrayBuffer = createReverse()
    // arrayBuffer = changePitch(1.33)
    audio.src = URL.createObjectURL(audioBufferToBlob(arrayBuffer, 'audio/wav'))
    // createReverb()
    // createReverbToUnderwater()

    async function createReverb () {
        const source = audioCtx.createMediaElementSource(audio)
        const convolver = audioCtx.createConvolver()
        const modifier = await fetch('question://./Musique/samples_audio_impulse-responses_matrix-reverb2.wav')
    ...
    async function createReverbToUnderwater () {
        const source = audioCtx.createMediaElementSource(audio)
        const convolver = audioCtx.createConvolver()
        const modifier = await fetch('question://./Musique/underwater.wav')
        const reverbBuffer = await modifier.arrayBuffer()
        convolver.buffer = await audioCtx.decodeAudioData(reverbBuffer)
    ...
    function changePitch (speedValue : number) {
